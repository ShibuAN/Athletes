document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Dashboard Data Fetching (On-Demand Strava)
        ...
        if (profile) {
            ...
            if (stravaStatusEl) {
                if (profile.strava_connected) {
                    ...
                    if (toggleBtn) {
                        toggleBtn.innerHTML = 'Refresh Data';
                        toggleBtn.className = 'btn btn-primary';
                        toggleBtn.style.display = 'inline-flex';

                        toggleBtn.onclick = async () => {
                            toggleBtn.disabled = true;
                            toggleBtn.innerHTML = '<i class="fa-solid fa-sync fa-spin"></i> Refreshing...';
                            try {
                                // Clear cache and force refresh
                                StravaAPI.clearCache(email);
                                await loadStravaActivities(email, profile, true);
                                toggleBtn.innerHTML = 'Refresh Data';
                            } catch (err) {
                        ...
                        // Add Unsync button
                        const buttonsContainer = document.getElementById('stravaButtonsContainer');
                        if (buttonsContainer && !document.getElementById('unsyncBtn')) {
                            const unsyncBtn = document.createElement('button');
                            unsyncBtn.id = 'unsyncBtn';
                            unsyncBtn.className = 'btn btn-outline';
                            unsyncBtn.style.fontSize = '0.85rem';
                            unsyncBtn.style.padding = '0.6rem 1.2rem';
                            unsyncBtn.innerHTML = 'Disconnect';
                            unsyncBtn.onclick = async () => {
                            ...
                            buttonsContainer.appendChild(unsyncBtn);
                        }
                    }
                    ...
                    // Clear UI if not connected
                    const totalActivitiesEl = document.getElementById('totalActivities');
                    if (totalActivitiesEl) totalActivitiesEl.textContent = '0';
                    const activityListEl = document.getElementById('activityList');
                    if (activityListEl) activityListEl.innerHTML = '<div style="text-align: center; padding: 3rem; color: var(--text-muted);">Connect Strava to see your performances.</div>';
                    return;
                }
            }
        }
    ...
    // Load Strava Activities On-Demand (No DB query)
    async function loadStravaActivities(email, profile, forceRefresh = false) {
        ...
        try {
            listEl.innerHTML = '<div style="text-align: center; padding: 3rem; color: var(--text-muted);"><i class="fa-solid fa-spinner fa-spin"></i> Loading activities...</div>';

            // Fetch on-demand from Strava API (with caching)
            const activities = await StravaAPI.getDashboardActivities(email, profile, 5);

            // Get this month's count
            const monthCount = await StravaAPI.getThisMonthCount(email, profile);
            if (countEl) countEl.textContent = monthCount;
            ...
            if (activities && activities.length > 0) {
                ...
                listEl.innerHTML = '<div style="text-align: center; padding: 3rem; color: var(--text-muted);">No activities found. Start exercising!</div>';
            }
        } catch (err) {
            console.error('Error loading Strava activities:', err);
            listEl.innerHTML = '<div style="text-align: center; padding: 3rem; color: #ef4444;">Failed to load activities. Try refreshing.</div>';
        }
    }
